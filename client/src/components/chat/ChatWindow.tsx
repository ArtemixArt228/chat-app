...
const ChatWindow: React.FC<ChatWindowProps> = ({ groupId, socket, user }) => {
    ...
    const validateSession = async () => {
        ...
        try {
            const { response: isValid } = await chatSessionService.validateSession({ sessionID });
            return isValid;
        } catch {
    ...
    const renderMessage = (message: any) => (
        <div key={message._id} className="mb-4">
            ...
            {message.isVoiceMessage ? (
                <audio controls src={`${process.env.REACT_APP_BASE_URL}/${message.fileURL}`} className="mt-1 w-1/3 rounded-md" />
            ) : (
